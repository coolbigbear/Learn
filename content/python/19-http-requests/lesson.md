...
- **Calling `.json()` on non-JSON responses** — If the API returns an error page (HTML), `response.json()` raises an error. Always check `response.ok` or `response.status_code` first.
- **Forgetting to set `Content-Type` header** — When sending JSON with `data=` instead of `json=`, the server won't know the format unless you set `Content-Type: application/json` manually.
- **Hardcoding URLs instead of using f-strings** — `f"https://api.example.com/users/{user_id}"` is cleaner and safer than string concatenation.
- **Not handling network errors** — API calls can fail due to network issues. Always wrap calls in try/except or use `raise_for_status()`.
- **Confusing path and query parameters** — Path parameters identify a resource (`/users/42`); query parameters filter or sort (`/users?role=admin`). They serve different purposes.

## Best Practices

1. **Use `json=` parameter** — Python's `requests` library handles serialization and sets `Content-Type` automatically when you use `json=payload`.
2. **Always set `Accept: application/json`** — It tells the server you expect a JSON response. Some APIs return different formats based on this header.
3. **Use f-strings for URL construction** — F-strings are the most readable way to build URLs with path variables in Python.
4. **Use dictionary unpacking for headers** — Combine default headers and request-specific headers with `{**default_headers, **extra_headers}`.
5. **Separate base URL from path** — Store the base URL (`https://api.example.com`) as a variable and append the path; makes your code easier to update.
...
Now that you know how to make HTTP requests from code, you're ready to understand what the server sends back. In the next lesson, you'll learn all about **HTTP response codes** — what they mean and how to handle them.
